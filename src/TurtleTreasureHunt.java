...
public class TurtleTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		if(tortoiseLocationX==490 && tortoiseLocationY==310){
		...
		if(tortoiseLocationX==145 && tortoiseLocationY==315){
			JOptionPane.showMessageDialog(null,"Go to the robot next...");
		}
		if(tortoiseLocationX==410 && tortoiseLocationY==295){
			JOptionPane.showMessageDialog(null,"Go to the torch...");
		}
		if(tortoiseLocationX==50 && tortoiseLocationY==195){
			JOptionPane.showMessageDialog(null,"Congratulations!");
			JOptionPane.showMessageDialog(null,"You found the secret passage to the treasure!");
		}
		// 7. Give the user subsequent clues at different locations on the image
		// (pirate robot, swamp, parrots, etc.)50 195
